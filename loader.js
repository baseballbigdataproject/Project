...
//     $.getJSON('https://jsonplaceholder.typicode.com/users', function(players) {
//       console.log(players)
...
script.onload = function(){
    $.getJSON('https://jsonplaceholder.typicode.com/users', function(players) {
      console.log(players)
      for(let i = 0; i < players['players'].length; i++) {
        $("#playerTable #tbody").append('<tr><th scope="row">' + (i + 1) + '</th><td>' + players["players"][i]["name"] + '</td><td>' + players["players"][i]["username"]  + '</td></tr>')
      }
    });
